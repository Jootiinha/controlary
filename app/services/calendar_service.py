...
CalendarEventType = Literal["pagamento", "renda", "assinatura", "fixo", "parcela"]

# Se não houver cartão ou dados legados, usar dia 1 do mês de referência.
...
def _months_from_until(d0: date, d1: date):
    """Meses (ano, mês) que intersectam o intervalo [d0, d1]."""
...
def events_for_month(ano: int, mes: int) -> list[CalendarEvent]:
    ...
    for inst in installments_service.list_all():
        ...
        if inst.mes_referencia != ano_mes:
            continue
        c = _get_card(inst.cartao_id)
        if c is not None:
            d = _data_com_dia(ano, mes, c.dia_pagamento_fatura)
            titulo = f"{inst.nome_fatura} — {c.nome} · parcela"
        else:
...
def upcoming_payables(horizon_days: int = UPCOMING_HORIZON_DAYS) -> list[CalendarEvent]:
    """Compromissos a pagar (assinatura, fixo pendente, parcela) entre hoje e hoje+horizon."""
    today = date.today()
    end = today + timedelta(days=horizon_days)
    out: list[CalendarEvent] = []
    for y, m in _months_from_until(today, end):
        for ev in events_for_month(y, m):
            if ev.tipo not in ("assinatura", "fixo", "parcela"):
                continue
