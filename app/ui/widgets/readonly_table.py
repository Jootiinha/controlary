        ...
        *,
        ...
        if not rows:
            if empty_row is not None:
                ...
                for c, text in enumerate(empty_row):
                    self.setItem(0, c, self._make_item(c, text))
                return
            if empty_message is not None:
                self.setRowCount(1)
                self.setItem(0, 0, self._make_item(0, empty_message))
                for c in range(1, ncols):
                    self.setItem(0, c, self._make_item(c, "—"))
                return
            self.setRowCount(0)
            return
        ...
        for r, row in enumerate(rows):
            ...
            for c, text in enumerate(row):
                self.setItem(r, c, self._make_item(c, text))

    def _make_item(self, column: int, text: str) -> QTableWidgetItem:
